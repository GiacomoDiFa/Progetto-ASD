...
int main(void)
{
	...
	appoggio_valori = (int *)calloc(n, sizeof(int));
	appoggio_stringhe = (char **)calloc(n, sizeof(char *));		
	...
	/* stampa a monitor dei dati contenuti nell'albero secondo un ordine di visita scelto dall’utente 
	tra anticipato, simmetrico e posticipato */
	...
	printf("\nI dati ordinati in base alla chiave numerica sono:\n"); 
	visita_albero_bin_simm(radice_p, &conta_simm2);
	...
	do
	{
		printf("\nInserisci il numero che vuoi cercare all'interno dell'albero: \n");
		esito_lettura = scanf("%d", &valore_ricerca);
		
		if (esito_lettura != 1 || valore_ricerca < 0)
			printf("Input non accettabile!\n");
		
		while (getchar() != '\n');
	}
	while (esito_lettura != 1 || valore_ricerca < 0);
	
	risultato_ricerca_p = cerca_in_albero_bin_ric(radice_p, valore_ricerca, &conta_cerca);
	
	printf("Il valore cercato ");
